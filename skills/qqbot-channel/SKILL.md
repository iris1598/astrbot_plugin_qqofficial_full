---
name: qqbot-channel
description: QQ 频道管理技能（非QQ群）。查询频道列表、子频道、成员、发帖、公告、日程等操作。使用 qqbot_platform_api 工具代理 QQ 开放平台 HTTP 接口，自动处理 Token 鉴权。仅用于 QQ 频道（Guild/Channel）场景。触发条件：用户提到「频道」「子频道」「guild」「channel」「帖子」「日程」「公告」。
---
...
> 基础 URL 默认为 `https://api.sgroup.qq.com`（沙箱/自建网关以适配器 `api_base_url` 配置为准），鉴权头 `Authorization: QQBot {token}` 由工具自动填充。
...
| 错误码 | 说明 | 解决方案 |
|--------|------|----------|
| **401** | Token 鉴权失败 | 检查适配器 AppID 和 AppSecret 配置 |
